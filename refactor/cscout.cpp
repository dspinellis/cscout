 ...
 *
 * $Id: cscout.cpp,v 1.139 2006/06/13 16:22:52 dds Exp $
 */
	...
	for (fun = Call::fbegin(); fun != Call::fend(); fun++) {
		...
		for (call = fun->second->call_begin(); call != fun->second->call_end(); call++) {
			...
			if (only_visited && !(fun->second->is_visited() && (*call)->is_visited()))
				continue;
			fprintf(fo,
			    html ? "<tr><td align=\"right\">%s</td><td>&rarr;</td><td>%s</td></tr>\n" : "%s %s\n",
			    function_label(fun->second, html).c_str(),
			    function_label(*call, html).c_str()
			);
		}
	}
	if (html) {
...
// Print the heading of a dot file
static void
dot_head(FILE *fo)
{
...
// Print a node of a dot file
static void
dot_node(FILE *fo, Call *p, bool hyperlink)
{
...
// Print an edge of a dot file
static void
dot_edge(FILE *fo, Call *a, Call *b)
{
	fprintf(fo, "\t_%p -> _%p;\n", a, b);
}
...
static void
image_page(FILE *fo, const char *format, void (*generator)(FILE *f, const char *format))
{
	prohibit_remote_access(fo);
...
{
	image_page(fo, "svg", cgraph_dot_page);
}
...
cgraph_gif_page(FILE *fo, void *p)
{
	...
	#endif
	image_page(fo, "gif", cgraph_dot_page);
}
